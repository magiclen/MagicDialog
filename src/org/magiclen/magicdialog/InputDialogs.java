...
public class InputDialogs implements Cloneable {
     ...
     * @return 再把對話框設定傳回
     */
    public InputDialogs fontFamily(final String fontFamily) {
    ...
    public Object clone() {
        ...
        dialogs.audio(audio).fontSize(fontSize).fontFamily(fontFamily).owner(owner).message(message).header(header).title(title).text(text);
        return dialogs;
    }
